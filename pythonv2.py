...
def insert_data(client, db_name, collection_name):
    ...
    existing_columns = collection.find_one()
    print("Mevcut sütunlar:", list(existing_columns.keys()))

    data = {}
    for column in existing_columns.keys():
        if column == '_id':
            continue
        data[column] = input(f"{column} değeri girin: ")
